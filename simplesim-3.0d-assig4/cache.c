...
/* Static table for stride prefetcher*/
//#define STRIDE_TABLE_SIZE 4096
//static stride_entry stride_table[STRIDE_TABLE_SIZE];
	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  if (!cp->data)
    ...
    // RPT for stride
    if (prefetch_type >= 2) {
        int rpt_size = prefetch_type;
        if (prefetch_type == 2) rpt_size = RPT_DEFAULT_SIZE;
        cp->rptable = calloc(1, sizeof(struct rpt_t));
        rpt *rptable = cp->rptable;
        rptable->tag = calloc(rpt_size, sizeof(md_addr_t));
        rptable->prev_addr = calloc(rpt_size, sizeof(md_addr_t));
        rptable->stride = calloc(rpt_size, sizeof(md_addr_t));
        rptable->stride_negative = calloc(rpt_size, sizeof(int));
        rptable->state = calloc(rpt_size, sizeof(size_t));
    }
    else {
        cp->rptable = NULL;
    }
...
void next_line_prefetcher(struct cache_t *cp, enum mem_cmd cmd, md_addr_t addr, tick_t now) {

  //Addr to prefetch...
  //  prefetch_ADDR = current_ADDR aligned to 64 bytes + cache_line_size
  md_addr_t prefetch_addr = ((addr >> 6) << 6) + cp->bsize; 
  //printf("current_ADDR=%d, prefetch_ADDR=%d, line_size=%d\n",
  //       addr, prefetch_addr, cp->bsize);

  //First check if next line is in the cache or not....
  if (!cache_probe(cp, prefetch_addr)) {
    //printf("next line not in cache... need to prefetch, now=%d\n", now);
  cache_access(cp, cmd, prefetch_addr, NULL,  cp->bsize, now, NULL, NULL, 1);
  }
	; 
}

/* Open Ended Prefetcher */
void open_ended_prefetcher(struct cache_t *cp, enum mem_cmd cmd, md_addr_t addr, tick_t now) {
	; 
}

double log2n(double n)
{
    return log(n)/log(2); // This is dependent on math.h
}
...
void prefetch(struct cache_t *cp, md_addr_t addr) {
    ...
    md_addr_t block_addr = CACHE_MK_BADDR(cp, tag, set);
    if (cache_probe(cp, block_addr)) return;
    cache_access(cp, Read, block_addr, NULL, cp->bsize, NULL, NULL, NULL, 1);
}

int next_state(int state, int stride) {
    if (stride != 0) {    
        switch (state) {
            case 0:
            case 1:
...
/* Stride Prefetcher */
void stride_prefetcher(struct cache_t *cp, enum mem_cmd cmd, md_addr_t addr, tick_t now) {
    assert(cp);
    
    int index_offset, tag_offset;
    size_t index;
    int rpt_size = cp->prefetch_type;
    //printf ("prefetch type %d\n", rpt_size);
    assert((rpt_size & (rpt_size-1)) == 0);
    
    md_addr_t tag;
    md_addr_t PC = get_PC();

    index_offset = log2n(sizeof(md_inst_t));
    size_t index_mask = rpt_size - 1;
    tag_offset = index_offset + log2n(rpt_size);
    index = (PC >> index_offset) & index_mask;      //RPT size is power of 2
    tag = PC>>tag_offset;

    rpt* rptable = cp->rptable;
    if (rptable->tag[index] == tag) {
        md_addr_t prev_addr = rptable->prev_addr[index];      
        int stride_negative = 0;
        md_addr_t new_stride = addr-prev_addr;
        if (new_stride < 0) {
        ...
        int same_stride = new_stride==rptable->stride[index];
        int same_direction = stride_negative == rptable->stride_negative[index];
        int stride_condition = same_stride && same_direction;
 
        rptable->state[index] = next_state(rptable->state[index], stride_condition);
        //Update stride if false stride condition and state is not initial
        if (!stride_condition &&
            (rptable->state[index] == 1 || rptable->state[index] == 3)) {
            rptable->stride[index] = new_stride;
            rptable->stride_negative[index] = stride_negative;
        }

        rptable->prev_addr[index] = addr;

        //Try to prefetch if state is not 3
        if (rptable->state[index] != 3) {
            md_addr_t prefetch_addr = rptable->stride_negative[index]?
            addr - rptable->stride[index]:
            addr + rptable->stride[index];
        ...
        //Insert an entry, may overwrite entry with same index.
        rptable->prev_addr[index] = addr;
        rptable->tag[index] = tag;
        rptable->stride[index] = 0;
        rptable->state[index] = 0;
        rptable->stride_negative[index] = 0;
        return;
    }
...
void generate_prefetch(struct cache_t *cp, enum mem_cmd cmd, md_addr_t addr, tick_t now) {
	...
	switch(cp->prefetch_type) {
		...
		case 1:
		   // Next Line Prefetcher
		   next_line_prefetcher(cp, cmd, addr, now);
		   break;
		case 2:
		   // Open Ended Prefetcher
		   open_ended_prefetcher(cp, cmd, addr, now);
		   break;
		default:
		   // Stride Prefetcher with cp->prefetch_type number of entries in the Reference Prediction Table (RPT)
		   stride_prefetcher(cp, cmd, addr, now);
	}
	     ...
	     int prefetch)		/* 1 if the access is a prefetch, 0 if it is not */
{
  ...
  if (prefetch == 0) {	/* only regular cache accesses can generate a prefetch */
  	generate_prefetch(cp, cmd, addr, now);
  }
